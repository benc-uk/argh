...
fn test_load_obj_icosahedron_succeeds() {
  ...
  let h = e.load_obj(p.to_str().unwrap()).expect("icosahedron.obj should load");
  let m = e.model(h);
...
fn test_load_obj_chest_loads_without_texture_in_mtl() {
  ...
  let h = e.load_obj(p.to_str().unwrap()).expect("chest.obj should load");
  let m = e.model(h);
...
fn test_load_obj_dice_carries_texture() {
  ...
  let h = e.load_obj(p.to_str().unwrap()).expect("dice.obj should load");
  let m = e.model(h);
...
fn test_load_obj_missing_file_returns_load_error() {
  let mut e = Engine::new(64, 64);
  match e.load_obj("definitely/does/not/exist.obj") {
...
fn test_load_obj_garbage_returns_load_error() {
  ...
  let result = e.load_obj(path.to_str().unwrap());
...
fn test_load_obj_adds_to_engine_model_registry() {
  ...
  let _ = e.load_obj(p.to_str().unwrap()).expect("skull.obj should load");
...
fn test_load_obj_name_uses_file_stem() {
  let mut e = Engine::new(64, 64);
  let p = asset_path("obj/skull.obj");
  let h = e.load_obj(p.to_str().unwrap()).expect("skull.obj should load");
