...
fn small_baked() -> BakedMesh {
  BakedMesh {
    ...
    verts: vec![Vec3::new(0.0, 0.0, 0.0), Vec3::new(1.0, 0.0, 0.0), Vec3::new(0.0, 1.0, 0.0)],
...
fn test_bake_lighting_populates_per_vert_entries() {
  ...
  let lights = lights_with(Light::new(Vec3::new(0.0, 0.0, 5.0), 1.0, WHITE, 0.0, 0.0, true, false));
...
fn test_bake_lighting_clears_previous_results() {
  let mut bm = small_baked();
  bm.baked_lighting = vec![Colour::new(99.0, 99.0, 99.0); 999]; // garbage
  let lights = empty_lights();
...
fn test_bake_lighting_ignores_non_static_lights() {
  ...
  let lights = lights_with(Light::new(Vec3::new(0.0, 0.0, 5.0), 1.0, WHITE, 0.0, 0.0, false, false));
  bm.bake_lighting(&lights, Colour::new(0.0, 0.0, 0.0));
  for c in &bm.baked_lighting {
    let (r, g, b) = c.channels();
...
fn test_bake_lighting_includes_static_lights() {
  ...
  let lights = lights_with(Light::new(Vec3::new(0.0, 0.0, 5.0), 1.0, WHITE, 0.0, 0.0, true, false));
  bm.bake_lighting(&lights, Colour::new(0.0, 0.0, 0.0));
  let (r, _, _) = bm.baked_lighting[0].channels();
  assert!(r > 0.0);
}
...
fn test_bake_lighting_attenuates_with_distance() {
  ...
  let near_lights = lights_with(Light::new(Vec3::new(0.0, 0.0, 1.0), 1.0, WHITE, 0.1, 0.1, true, false));
  let far_lights = lights_with(Light::new(Vec3::new(0.0, 0.0, 100.0), 1.0, WHITE, 0.1, 0.1, true, false));
  near_bm.bake_lighting(&near_lights, Colour::new(0.0, 0.0, 0.0));
  far_bm.bake_lighting(&far_lights, Colour::new(0.0, 0.0, 0.0));
  let near_r = near_bm.baked_lighting[0].channels().0;
  let far_r = far_bm.baked_lighting[0].channels().0;
